...
//初始化区域图的对象
	...
	if(property.sizeAble){
        width=(property.width||100)-2;
        height=(property.height||100)-2;
	...
	if(property.haveTool){
		this.$bgDiv.append("<div style='position: fixed;left: -1000px;' class='GooFlow_tool'"+(property.haveHead? "":" style='margin-top:3px'")+"><div style='height:"+(height-headHeight-(property.haveHead? 7:10))+"px' class='GooFlow_tool_div'></div></div>");
	...
	this.$bgDiv.append("<div class='GooFlow_work' id='GooFlow_work' style='width:"+(width)+"px;height:"+(height)+"px;"+(property.haveHead? "":"margin-top:3px")+"'></div>");
	...
	if(this.$editable){
	  ...
	  var delete_btn = GooFlow.property.canEdit? "<b class='b_x'></b>":"";
	  this.$lineOper=$("<div class='GooFlow_line_oper' style='display:none'>" +
		  "<b class='b_l1'></b>" +
		  "<b class='b_l2'></b>" +
		  "<b class='b_l3'></b>" +
		  delete_btn +
	  ...
	  //下面绑定当结点/线/分组块的一些操作事件,这些事件可直接通过this访问对象本身
        ...
        this.onItemAdd=null;
        //当操作某些单元（结点/线/分组块）被添加时，触发的方法
        //格式function(ids，type)：ids是单元的唯一标识ID的数组,type是单元的种类,有"node","line","area"三种取值
        this.onItemsAdd=null;
	  //当操作某个单元（结点/线/分组块）被删除时，触发的方法，返回FALSE可阻止删除事件的发生
	  //格式function(id，type)：id是单元的唯一标识ID,type是单元的种类,有"node","line","area"三种取值
	  this.onItemDel=null;
        //当操作某些单元（结点/线/分组块）被删除时，触发的方法
        //格式function(ids，type)：ids是单元的唯一标识ID的数组,type是单元的种类,有"node","line","area"三种取值
        this.onItemsDel=null;
        //当操作某个单元（结点）在其前项添加节点时，触发的方法，返回FALSE可阻止事件的发生
        //格式function(id，type)：id是单元的唯一标识ID,type是单元的种类,有"node"一种取值
        this.onNodeAddPre=null;
        //当操作某个单元（结点）在其后项添加节点时，触发的方法，返回FALSE可阻止事件的发生
        //格式function(id，type)：id是单元的唯一标识ID,type是单元的种类,有"node"一种取值
        this.onNodeAddNext=null;
...
GooFlow.prototype={
	...
	initDraw:function(id,width,height){
		...
		if(GooFlow.prototype.useSVG!=""){
			...
			defs.appendChild(GooFlow.prototype.getSvgMarker("arrow1","#000000"));
	...
	addNode:function(id,json){
		...
		if(json.type!="start"&&json.type!="end"){
            ...
            var delete_btn = GooFlow.property.canEdit? "<div class='rs_close'></div>" : "";
            //var delete_btn_2 = GooFlow.property.canEdit? "<div class='rs_close_2'></div>" : "";
            var insert_btn = GooFlow.property.canEdit? "<div class='rs_insertPre'></div><div class='rs_insertNext'></div>" : "";
			var time_content = GooFlow.property.type == "wtg"? "<tr><td style='border: 1px solid; color: white; background-color: "+json.time_color+";border-color: "+json.time_color+"'>完成："+json.time_end+"</td></tr>" +
				"<tr><td style='border: 1px solid;color: white;background-color: "+json.time_color+";border-color: "+json.time_color+"'>持续："+json.time+"天</td></tr>" : "";
			var time_btn = GooFlow.property.type == "wtg"?"<div class='rs_time'></div>":"";
			this.$nodeDom[id]=$("<div class='GooFlow_item"+mark+"' id='"+id+"' style='top:"+json.top+"px;left:"+json.left+"px;width:"+(json.width-2)+"px;height:"+(json.height-2)+"px;'>"
				// +"<div style='position: absolute;top: -16px;width: 100%;text-align: center;'>\"+json.code+\"</div>"
				+"<table class='photo_dbclick' cellspacing='1' style='width:"+(json.width-2)+"px;position: absolute; left: -1px; top: -"+(json.height-2)/2+"px'><tr>"
				+"<td  class='ico' style='height:"+(json.height-2)/2+"px;'>"+json.code+"</td></tr><tr>"
				+"<td class=ico' style='padding: 6px; height:"+(json.height-2)+"px;'>"+json.name+"</td></tr>"
				+time_content
	...
	initWorkForNode:function(){
		...
		this.$workArea.delegate(".GooFlow_item","mouseup",{inthis:this},function(e){
			...
			if(lineStart)	This.addLine(lineStart.id+'_'+this.id,{from:lineStart.id,to:this.id,name:""});
			This.$max++;
		});
		//绑定双击编辑事件
        ...
        //绑定时间控件点击事件
        this.$workArea.delegate(".rs_time","click",{inthis:this},function(e){
            if(!e)e=window.event;
            e.data.inthis.time_click(e.data.inthis.$focus);
            return false;
		});
		this.$workArea.delegate(".photo_dbclick","dblclick",{inthis:this},function(e){
            if(!e)e=window.event;
            e.data.inthis.photo_dbclick(e.data.inthis.$focus);
            return false;
        });
        //绑定结点的删除功能(拓扑关系删除)
        ...
        //绑定结点的删除功能(拓扑关系继承)
        this.$workArea.delegate(".rs_close_2","click",{inthis:this},function(e){
            if(!e)e=window.event;
            e.data.inthis.delNode_2(e.data.inthis.$focus);
            return false;
        });
		//绑定节点前项插入节点功能
        this.$workArea.delegate(".rs_insertPre","click",{inthis:this},function(e){
            if(!e)e=window.event;
            e.data.inthis.addNodePreNode(e.data.inthis.$focus);
            return false;
        });
        //绑定节点后项插入节点功能
        this.$workArea.delegate(".rs_insertNext","click",{inthis:this},function(e){
            if(!e)e=window.event;
            e.data.inthis.addNodeNextNode(e.data.inthis.$focus);
            return false;
        });
		//绑定结点的RESIZE功能
    ...
    time_click:function(id){
        ...
        if(this.onTimeClick!=null){
            this.onTimeClick(id,"node");
		}
	},
	photo_dbclick:function(id) {
		if(!this.$nodeData[id])	return;
        if(this.$undoStack){
            var paras=[id,this.$nodeData[id]];
            this.pushOper("addNode",paras);
        }
        if(this.onPhotoDbclick!=null){
            this.onPhotoDbclick(id,"node");
		}
	},
    //删除结点（拓扑关系删除）
    delNode:function(id){
    ...
    //删除结点(拓扑关系继承)
    delNode_2:function(id){
        if(!this.$nodeData[id])	return;
        if(this.$undoStack){
            var paras=[id,this.$nodeData[id]];
            this.pushOper("addNode",paras);
        }
        if(this.onItemDel!=null&&!this.onItemDel(id,"node_2"))	return;
        delete this.$nodeData[id];
        this.$nodeDom[id].remove();
        delete this.$nodeDom[id];
        --this.$nodeCount;
    ...
    //向一个节点前添加一个结点
    addNodePreNode:function(id){
        if(!this.$nodeData[id])	return;
        if(this.$undoStack){
            var paras=[id,this.$nodeData[id]];
            this.pushOper("addNode",paras);
        }
        if(this.onNodeAddPre!=null&&!this.onNodeAddPre(id,"node"))	return;
    },
    //向一个节点后添加一个结点
    addNodeNextNode:function(id){
        if(!this.$nodeData[id])	return;
        if(this.$undoStack){
            var paras=[id,this.$nodeData[id]];
            this.pushOper("addNode",paras);
        }
        if(this.onNodeAddNext!=null&&!this.onNodeAddNext(id,"node"))	return;
    },
	//设置流程图的名称
...
///////////以下为有关画线的方法
    ...
    //增加一条线
    addLines:function(ids,jsons){
    	for(var i=0; i<ids.length; i++){
            if(this.$undoStack){
                this.pushOper("delLine",[ids[i]]);
            }
            var n1=null,n2=null;//获取开始/结束结点的数据
            if(jsons[i].from==jsons[i].to)	return;
            //避免两个节点间有两条以下直接连线
            for(var k in this.$lineData){
                if((jsons[i].from==this.$lineData[k].from&&jsons[i].to==this.$lineData[k].to)
                    ||(jsons[i].from==this.$lineData[k].to&&jsons[i].to==this.$lineData[k].from))
                    return;
            }
            var n1=this.$nodeData[jsons[i].from],n2=this.$nodeData[jsons[i].to];//获取开始/结束结点的数据
            if(!n1||!n2)	return;
            var res;
            if(jsons[i].type&&jsons[i].type!="sl")
                res=GooFlow.prototype.calcPolyPoints(n1,n2,jsons[i].type,jsons[i].M);
            else
                res=GooFlow.prototype.calcStartEnd(n1,n2);
            if(!res)	return;
            this.$lineData[ids[i]]={};
            if(jsons[i].type){
                this.$lineData[ids[i]].type=jsons[i].type;
                this.$lineData[ids[i]].M=jsons[i].M;
            }
            else	this.$lineData[ids[i]].type="sl";//默认为直线
            this.$lineData[ids[i]].from=jsons[i].from;
            this.$lineData[ids[i]].to=jsons[i].to;
            this.$lineData[ids[i]].name=jsons[i].name;
            if(jsons[i].mark)	this.$lineData[ids[i]].marked=jsons[i].mark;
            else	this.$lineData[ids[i]].marked=false;

            if(this.$lineData[ids[i]].type=="sl")
                this.$lineDom[ids[i]]=GooFlow.prototype.drawLine(ids[i],res.start,res.end,jsons[i].mark);
            else
                this.$lineDom[ids[i]]=GooFlow.prototype.drawPoly(ids[i],res.start,res.m1,res.m2,res.end,jsons[i].mark);
            this.$draw.appendChild(this.$lineDom[ids[i]]);
            if(GooFlow.prototype.useSVG==""){
                this.$lineDom[ids[i]].childNodes[1].innerHTML=jsons[i].name;
                if(this.$lineData[ids[i]].type!="sl"){
                    var Min=(res.start[0]>res.end[0]? res.end[0]:res.start[0]);
                    if(Min>res.m2[0])	Min=res.m2[0];
                    if(Min>res.m1[0])	Min=res.m1[0];
                    this.$lineDom[i].childNodes[1].style.left = (res.m2[0]+res.m1[0])/2-Min-this.$lineDom[ids[i]].childNodes[1].offsetWidth/2+4;
                    Min=(res.start[1]>res.end[1]? res.end[1]:res.start[1]);
                    if(Min>res.m2[1])	Min=res.m2[1];
                    if(Min>res.m1[1])	Min=res.m1[1];
                    this.$lineDom[ids[i]].childNodes[1].style.top = (res.m2[1]+res.m1[1])/2-Min-this.$lineDom[ids[i]].childNodes[1].offsetHeight/2;
                }else
                    this.$lineDom[ids[i]].childNodes[1].style.left=
                        ((res.end[0]-res.start[0])*(res.end[0]>res.start[0]? 1:-1)-this.$lineDom[ids[i]].childNodes[1].offsetWidth)/2+4;
            }
            else	this.$lineDom[ids[i]].childNodes[2].textContent=jsons[i].name;
            ++this.$lineCount;
		}

        if(this.onItemsAdd!=null){
            this.onItemsAdd(ids,"line");
		}
    },
    ...
    //删除转换线（批量）
    delLines:function(ids){
    	for(var i=0; i<ids.length; i++){
            if(!this.$lineData[ids[i]])	continue;
            if(this.$undoStack){
                var paras=[ids[i],this.$lineData[ids[i]]];
                this.pushOper("addLine",paras);
            }
            this.$draw.removeChild(this.$lineDom[ids[i]]);
            delete this.$lineData[ids[i]];
            delete this.$lineDom[ids[i]];
            if(this.$focus==ids[i])	this.$focus="";
            --this.$lineCount;
		}
		if(this.onItemsDel != null){
            this.onItemsDel(ids, "line");
		}
    },
